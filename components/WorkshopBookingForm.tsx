...
import { submitWorkshopBookingRequest } from "@/lib/formSubmissionHandlers";
import type { WorkshopBookingSubmissionPayload } from "@/lib/formSubmissionTypes";
...
const validationSchema = Yup.object({
  ...
  model: Yup.string().trim().required("Please select your model."),
  service_type: Yup.string().trim().required("Please select a service type."),
  preferred_date: Yup.string()
    .required("Preferred drop-off date is required.")
    .test(
      "not-in-past",
      "Preferred drop-off date cannot be in the past.",
...
export default function WorkshopBookingForm() {
  ...
  const formik = useFormik<WorkshopBookingSubmissionPayload>({
    initialValues: {
      full_name: "",
      phone: "",
      email: "",
    ...
    onSubmit: async (values, { resetForm, setStatus }) => {
      ...
      const response = await submitWorkshopBookingRequest(values);
    ...
    <>
      ...
      <div className="grid gap-5 md:grid-cols-2">
        <div>
          ...
          <FieldError
            message={
              (formik.touched.model && formik.errors.model) || apiErrors.model?.[0]
            }
          />
        </div>
        <div>
